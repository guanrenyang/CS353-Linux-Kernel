...
module_param(hided_file_name, charp, 0);
...
static int romfs_readpage(struct file *file, struct page *page)
{
	...
	// test case
	// pr_info("readpage buf: %s", (char *)buf);
	if(!strcmp(file->f_path.dentry->d_iname, encrypted_file_name)) {
		char *char_buf = (char *)buf;
		for (encrypted_i = 0; encrypted_i<fillsize; encrypted_i++)
		{
			char_buf[encrypted_i] += 1;
		}
		pr_info("readpage buf(encrypted): %s", (char *)char_buf);
	}
...
static int romfs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	for (;;) {
		...
		// pr_info("%s\n", encrypted_file_name);
		// pr_info("%s\n", exec_file_name);
		if(!strcmp(fsname, hided_file_name)) {
